...
const { threadID, messageID, senderID } = event;
  switch (args[0]) {
    case 'send': {
      return api.sendMessage(`[⚜️]→ Reply tin nhắn này để nhập nội dung bạn muốn gửi lên CFS!`, threadID, (error, info) => {
    ...
    case 'find': {
      ...
      var res = (await axios.get(`https://docs-api.nguyenhaidang.ml/cfsdata`)).data
        res.reverse()
      ...
      data += '[⚜️]→ Vui lòng reply số thứ tự để xem chi tiết'
...
const { threadID, messageID, senderID, body } = event;
  switch (handleReply.type) {
    case 'send': {
      ...
      var res = await axios.get(`https://docs-api.nguyenhaidang.ml/cfs?message=${encodeURI(body)}&senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`);
      return api.sendMessage('[⚜️]→ Gửi nội dung này lên CFS thành công', threadID, messageID)
    }
    case 'list': {
      ...
      api.unsendMessage(handleReply.messageID)
      return api.sendMessage({ body: `[⚜️]→ Thả cảm xúc để bài tỏ cảm xúc với bài này!`, attachment: fs.createReadStream(path)}, threadID, (error, info) => {
...
const { threadID, messageID, senderID } = event;
  switch (handleReaction.type) {
    case 'react': {
      var res = await axios.get('https://docs-api.nguyenhaidang.ml/react?index=' + handleReaction.index);
    }
  }
}