...
module.exports.run = async function ({ api, event, Threads, getText, args }) {
  ...
  if (args[0] == "skill") {
    try {  
      let res = (await	axios.get(encodeURI(`https://dungkon.onrender.com/skinlq`+ args.join(" ").slice(6)))).data;
    ...
    try {  
    let res = (await	axios.get(encodeURI(`https://dungkon.onrender.com/skinlq`+ args.join(" ")))).data;
