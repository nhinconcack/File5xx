...
module.exports.handleReply = async function ({ event, api, handleReply }) {
  ...
  switch (handleReply.type) {
    case "jrt": {
      ...
      return api.sendMessage(`[⚜️]→ Bạn đã chọn chữ nền là ${event.body}\n\n[⚜️]→ Reply tin nhắn này nhập vào chữ ký của bạn`,threadID, function (err, info) {
    ...
    case "color": {
      ...
      var names = handleReply.names;
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`[⚜️]→ Bạn đã chọn chữ ký : ${event.body}\n\n[⚜️]→ Nhập màu của bạn (lưu ý: nhập tên tiếng anh của màu - Nếu không muốn nhập màu thì nhập "no")`,threadID, function (err, info) {
        return global.client.handleReply.push({
                  type: "create",
                  name,
                  author: senderID,
...
return api.sendMessage(`[⚜️]→ Nhân Vật: ${text} \n[⚜️]→ ID: ${idz}\n[⚜️]→ Màu Mặc Định: ${color}\n[⚜️]→ Nguồn: ${source}`, event.threadID, event.messageID)
 ...
 return api.sendMessage(`[⚜️]→ Đã tìm thấy ID nhân vật : ${id}\n[⚜️]→ Name nhân vật là ${names}\n\n[⚜️]→ Reply tin nhắn này và chọn chữ nền cho hình ảnh của bạn`,event.threadID, (err, info) => {
   return global.client.handleReply.push({
          type: "jrt",
          name,
          author: senderID,
