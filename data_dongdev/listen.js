...
logger("┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓", "[ PCODER ]");
  for(let i = 0; i <= admin.length -1; i++){
    dem = i + 1
    logger(` ID ADMIN ${dem}: ${(!admin[i]) ? "Trống" : admin[i]}`, "[ PCODER ]");
  }
  ...
  if ((event.body || '').startsWith(prefix) && event.senderID != api.getCurrentUserID() && !global.config.ADMINBOT.includes(event.senderID)  && !global.config.NDH.includes(event.senderID)) {
   ...
   try {
     ...
     if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'bank') != event.args[0]) {
        if (!find_thuebot) return api.shareContact(`💸Thuê bot đi r dùng\n📝Chỉ 30k-1th\n🙅liên hệ admin để thuê`,global.config.NDH[0], threadID);
        if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return api.shareContact(`💸Thuê bot đi r dùng\n📝Chỉ 30k-1th\n🙅liên hệ admin để thuê`,global.config.NDH[0], threadID);
     };
  };
